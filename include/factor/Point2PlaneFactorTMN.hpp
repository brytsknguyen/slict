...
class Point2PlaneFactorTMN
{
public:

    // Destructor
    ~Point2PlaneFactorTMN() {};

    // Constructor
    Point2PlaneFactorTMN(const Vector3d &finW_, const Vector3d &f_, const Vector4d &coef, double w_, int N_, double Dt_, double s_)
    :   finW       (finW_            ),
        f          (f_               ),
        n          (coef.head<3>()   ),
        m          (coef.tail<1>()(0)),
        ...
        s          (s_               )
    {
        ...
        // Jacobian
        jacobian = Matrix<double, 1, -1>::Zero(1, N*6);
    }
    ...
    bool Evaluate(vector<SO3d> &Rot, vector<Vector3d> &Pos, bool computeJacobian = true)
    {
        ...
        // Residual
        residual = (n.dot((R_W_Bt * f + p_inW_Bt)) + m) * w;
        ...
        {
            int ridx = j*6;
            Eigen::Block<Matrix<double, 1, -1>, 1, 3> J_knot_R = jacobian.block<1, 3>(0, ridx);
            J_knot_R.block<1, 3>(0, 0) = ddis_dR[j] * w;
        }
        ...
        {
            int pidx = j*6 + 3;
            Eigen::Block<Matrix<double, 1, -1>, 1, 3> J_knot_p = jacobian.block<1, 3>(0, pidx);
            J_knot_p.block<1, 3>(0, 0) = ddis_dP[j] * w;
        }
